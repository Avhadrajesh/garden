...
var ZKNavigationSettngTab = class extends import_obsidian7.PluginSettingTab {
  ...
  async display() {
    ...
    new import_obsidian7.Setting(indexButtonDiv).setName(t("Indexes folder location")).addSearch((cb) => {
      new FolderSuggest(this.app, cb.inputEl);
    ...
    new import_obsidian7.Setting(indexGraphView).setName(t("Index graph styles")).addExtraButton((cb) => {
      cb.setIcon("settings").onClick(() => {
        this.hideDiv(branchSectionDiv);
      });
    });
    const branchSectionDiv = indexGraphView.createDiv("zk-local-section zk-hidden");
    new import_obsidian7.Setting(branchSectionDiv).setName(t("direction of graph")).addDropdown((options) => options.addOption("LR", t('"LR": feft to right')).addOption("RL", t('"RL": right to left')).addOption("TB", t('"TB": top to bottom')).addOption("BT", t('"BT": bottom to top')).setValue(this.plugin.settings.DirectionOfBranchGraph).onChange((value) => {
      this.plugin.settings.DirectionOfBranchGraph = value;
      this.plugin.RefreshIndexViewFlag = true;
    }));
    new import_obsidian7.Setting(branchSectionDiv).setName(t("siblings order")).setDesc(t("siblings order description")).addDropdown((options) => options.addOption("number", t("number first")).addOption("letter", t("letter first")).setValue(this.plugin.settings.siblingsOrder).onChange((value) => {
      this.plugin.settings.siblingsOrder = value;
      this.plugin.RefreshIndexViewFlag = true;
    }));
    new import_obsidian7.Setting(branchSectionDiv).setName(t("same width for siblings")).addToggle((toggle) => toggle.setValue(this.plugin.settings.siblingLenToggle).onChange((value) => {
      this.plugin.settings.siblingLenToggle = value;
      this.plugin.RefreshIndexViewFlag = true;
    }));
    new import_obsidian7.Setting(branchSectionDiv).setName(t("Set red dash line for nodes with ID ends with letter")).setDesc(t("In order to distinguish nodes which ID ends with letter and number")).addToggle((toggle) => toggle.setValue(this.plugin.settings.RedDashLine).onChange((value) => {
      this.plugin.settings.RedDashLine = value;
      this.plugin.RefreshIndexViewFlag = true;
    }));
    new import_obsidian7.Setting(branchSectionDiv).setName(t("display created time")).setDesc(t("Set datetime format")).addText((cb) => {
    ...
    new import_obsidian7.Setting(branchSectionDiv).setName(t("Fold node toggle")).setDesc(t("Open the fold icon(\u{1F7E1}\u{1F7E2})")).addToggle((toggle) => toggle.setValue(this.plugin.settings.FoldToggle).onChange((value) => {
      this.plugin.settings.FoldToggle = value;
      this.plugin.RefreshIndexViewFlag = true;
    }));
    new import_obsidian7.Setting(branchSectionDiv).setName(t("Set color for nodes")).addColorPicker((color) => color.setValue(this.plugin.settings.nodeColor).onChange((value) => {
    ...
    new import_obsidian7.Setting(branchToolbarDiv).setName(t("settings")).addToggle((toggle) => toggle.setValue(this.plugin.settings.settingIcon).onChange((value) => {
      this.plugin.settings.settingIcon = value;
      this.plugin.RefreshIndexViewFlag = true;
    }));
    new import_obsidian7.Setting(branchToolbarDiv).setName(t("export to canvas")).addExtraButton((cb) => {
      cb.setIcon("settings").onClick(() => {
    ...
    if (this.plugin.settings.IndexButton == true) {
      new import_obsidian7.Setting(branchToolbarDiv).setName(t("random index")).addToggle((toggle) => toggle.setValue(this.plugin.settings.RandomIndex).onChange((value) => {
        this.plugin.settings.RandomIndex = value;
        this.plugin.RefreshIndexViewFlag = true;
      }));
    }
    if (this.plugin.settings.MainNoteButton == true) {
      new import_obsidian7.Setting(branchToolbarDiv).setName(t("random main note")).addToggle((toggle) => toggle.setValue(this.plugin.settings.RandomMainNote).onChange((value) => {
        this.plugin.settings.RandomMainNote = value;
        this.plugin.RefreshIndexViewFlag = true;
      }));
    }
    new import_obsidian7.Setting(branchToolbarDiv).setName(t("all trees")).addToggle((toggle) => toggle.setValue(this.plugin.settings.showAllToggle).onChange((value) => {
      this.plugin.settings.showAllToggle = value;
      this.plugin.RefreshIndexViewFlag = true;
    }));
    new import_obsidian7.Setting(branchToolbarDiv).setName(t("growing animation")).addToggle((toggle) => toggle.setValue(this.plugin.settings.play).onChange((value) => {
      this.plugin.settings.play = value;
      this.plugin.RefreshIndexViewFlag = true;
    }));
    new import_obsidian7.Setting(branchToolbarDiv).setName(t("play controller")).addToggle((toggle) => toggle.setValue(this.plugin.settings.playControllerToggle).onChange((value) => {
      this.plugin.settings.playControllerToggle = value;
      this.plugin.RefreshIndexViewFlag = true;
    }));
    new import_obsidian7.Setting(branchToolbarDiv).setName(t("table view")).addToggle((toggle) => toggle.setValue(this.plugin.settings.TableView).onChange((value) => {
      this.plugin.settings.TableView = value;
      this.plugin.RefreshIndexViewFlag = true;
    }));
    new import_obsidian7.Setting(branchToolbarDiv).setName(t("list tree")).addToggle((toggle) => toggle.setValue(this.plugin.settings.ListTree).onChange((value) => {
      this.plugin.settings.ListTree = value;
      this.plugin.RefreshIndexViewFlag = true;
    }));
    new import_obsidian7.Setting(branchToolbarDiv).setName(t("History List")).setDesc(t("And set the list length")).addSlider((cb) => {
    ...
    new import_obsidian7.Setting(indexGraphView).setName(t("Node menu")).addExtraButton((cb) => {
      cb.setIcon("settings").onClick(() => {
    ...
    new import_obsidian7.Setting(multiIDDiv).setName(t("Specify a frontmatter field(array) for multiple IDs")).addText((cb) => cb.setValue(this.plugin.settings.multiIDField).onChange((value) => {
...
`;
  ...
  onOpen() {
    ...
    import_obsidian11.MarkdownRenderer.render(this.app, this.data, contentDiv, "", this.plugin);
  ...
  appendTableLine() {
    for (let node of this.tableArr) {
      ...
      this.data = this.data + `|[[${node.ID}]]|${node.title}|${inlinksStr}|${outlinkStr}|${(0, import_obsidian11.moment)(node.ctime).format(this.plugin.settings.datetimeFormat)}|
`;
    }
  }
  ...
  async refreshBranchMermaid() {
    ...
    if (this.plugin.settings.BranchToolbra == true) {
      ...
      if (this.plugin.settings.play == true) {
        const playBtn = new import_obsidian12.ExtraButtonComponent(toolButtonsDiv);
      ...
      if (this.plugin.settings.playControllerToggle === true) {
        ...
        previousBtn.setIcon("arrow-left").onClick(async () => {
        ...
        nextBtn.setIcon("arrow-right").onClick(async () => {
        ...
        fullScreenBtn.setIcon("fullscreen").onClick(() => {
          let toggleClassList = [
        ...
        if (indexMermaid !== null) {
          ...
          if (flowchartG !== null) {
            ...
            for (let i2 = 0; i2 < nodeArr.length; i2++) {
              ...
              nodeArr[i2].addEventListener("contextmenu", (event) => {
